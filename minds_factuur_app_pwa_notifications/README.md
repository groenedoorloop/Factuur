...
Deze versie bevat:
- `mise.toml` met `python.github_attestations = false`
- `Procfile` met `--bind 0.0.0.0:$PORT`
- `railway.json` met dezelfde start command

Als Railway nog steeds een melding toont over subscription/payment, moet dat eerst in Railway zelf opgelost worden. Dat staat los van de code.


## Nieuw: CRM meldingsysteem

Deze versie heeft nu een intern meldingsysteem binnen het CRM:

- Rode melding bovenaan als facturen over datum zijn
- Waarschuwing als facturen binnen 3 dagen verlopen
- Totaalbedrag over datum
- Aantal verlopen facturen
- Tijdsklok per factuur:
  - X dagen verlopen
  - Vervalt vandaag
  - Vervalt morgen
  - X dagen resterend

Er worden geen externe e-mails of WhatsApp-berichten verstuurd. De meldingen verschijnen binnen het dashboard zodra je de app opent.
